...
public class CustomerAction extends BaseAction<Customer> {
	...
	public String sendSms() throws IOException {
		...
		System.out.println("session中的验证码为::"+checkcodeSession);
		// 编辑短信内容
	...
	public String activeMail() throws IOException {
		...
		if (activecodeRedis == null || !activecodeRedis.equals(activecode)) {
			...
			Customer customer = WebClient.create("http://localhost:8888/crm_management/services"
					+ "/customerService/customer/telephone/" + model.getTelephone()).accept(MediaType.APPLICATION_JSON)
					.get(Customer.class);
			if (customer.getType() == null || customer.getType() != 1) {
				// 没有绑定,进行绑定
				WebClient.create("http://localhost:8888/crm_management/services"
